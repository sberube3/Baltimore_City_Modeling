...
complete_R017<- read.csv("scenario1A_lowR0_uncontrolled.csv")
complete_R021<- read.csv("scenario1B_lowR0_MildDistancing.csv")
complete_R025<- read.csv("scenario1C_lowR0_ModDistancing.csv")
...
counts_array_R0val[which(complete_data$time==l),i,4]<- 
 ...
 for(m in 1:n_days){
   if(m<=5){ ##this is time spent on a ventilator until death ##change 5 here (rounded up from 4.5 days)
 ...
 else if(m>5&m<=8){ ##you need to also change the 5 here to adjust the time spent on a ventilator until death; the 8 here represents the time spent in ICU until death - if you change this you need to also change the 8 below in line 193
   counts_array_R0val[which(complete_data$time==m),i,5]<- 
      (counts_array_R0val[which(complete_data$time==(m)),i,3]*prob_matrix[i-1,4])+(counts_array_R0val[which(complete_data$time==(m-5)),i,4]*prob_matrix[i-1,6]) ##and you need to change the 5 here to adjust days spent on ventilator until death
 }
 else{
   counts_array_R0val[which(complete_data$time==m),i,5]<- 
      (counts_array_R0val[which(complete_data$time==(m-8)),i,3]*prob_matrix[i-1,4])+(counts_array_R0val[which(complete_data$time==(m-5)),i,4]*prob_matrix[i-1,6]) ##and you need to change the 5 here to adjust days spent on ventilator until death
 }
 }
}
...
symptomatic_df_sep<- symptomatic_df%>%
  ...
  mutate(scenario=ifelse(scenario=="R_0=1.7", "Low Transmission",
                         ifelse(scenario=="R_0=2.1", "Moderate Transmission", 
                                "High Transmission")))%>%
  mutate(Day=rep(seq(as.Date('2020/03/1'),by='day',length.out=n_days),48))
...
cumulative_cases_symptomatic_long<- cumulative_cases_symptomatic%>%
  ...
  mutate(Day= rep(seq(as.Date('2020/03/1'),by="day", length.out = n_days),3))
