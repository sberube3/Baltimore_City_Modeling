...
##load data
setwd("~/Desktop/Hopkins/Hopkins/COVID_baltimore/Baltimore_City_Modeling")
...
f_1[[6]]<- as.data.frame(cbind(f_1[[6]], rep(0,nrow(f_1[[6]]))))
colnames(f_1[[6]])<- c("Time", "scenario", "summary_stat", "incident_symptomatic",
                       "number")
incident_cases_df<- as.data.frame(rbind(f_1[[6]],f_1[[7]]))
...
f_1_asymp[[6]]<- as.data.frame(cbind(f_1_asymp[[6]],rep(0,nrow(f_1_asymp[[6]]))))
colnames(f_1_asymp[[6]])<- c("Time", "scenario", "summary_stat", "incident_symptomatic",
                       "number")
incident_cases_asymp_df<- as.data.frame(rbind(f_1_asymp[[6]],f_1_asymp[[7]]))
...
df_notE_hosp <- df_sep %>% dplyr::filter(scenario!="Uncontrolled")
random_df_notE_hosp <- random_df_sep %>% dplyr::filter(scenario!="Uncontrolled")
plt_cumulative_hosp<- ggplot() + 
  geom_line(data=df_notE_hosp,aes(y = cumulative_hosp, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_notE_hosp,aes(y = cumulative_hosp, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
df_notE_ICU <- df_sep %>% dplyr::filter(scenario!="Uncontrolled")
random_df_notE_ICU <- random_df_sep %>% dplyr::filter(scenario!="Uncontrolled")
plt_cumulative_ICU<- ggplot() + 
  geom_line(data=df_notE_ICU,aes(y = cumulative_ICU, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_notE_ICU,aes(y = cumulative_ICU, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
df_notE_vent <- df_sep %>% dplyr::filter(scenario!="Uncontrolled")
random_df_notE_vent <- random_df_sep %>% dplyr::filter(scenario!="Uncontrolled")
plt_cumulative_vent<- ggplot() + 
  geom_line(data=df_notE_vent,aes(y = cumulative_vent, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_notE_vent,aes(y = cumulative_vent, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
return(list(plt_cumulative_hosp,plt_cumulative_ICU,plt_cumulative_vent,df_notE_hosp,random_df_notE_hosp,df_notE_ICU,random_df_notE_ICU,df_notE_vent,random_df_notE_vent))
...
write.csv(f_1[[8]],"Figure_5_cumulative_cases.csv")
#cumsum deaths
