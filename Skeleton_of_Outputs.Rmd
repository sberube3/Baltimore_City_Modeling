...
prob_severe<- cbind(rep(0.0256,500),rep(0.263,500),rep(0.395,500),
                    rep(0.577,500),rep(1.732,500),rep(0.577,500),
                    rep(0.577,500))

prob_hospitalized<- matrix(1,nrow=500,ncol=7)

prob_ICU<- cbind(rep(0,500),rep(0.253,500),rep(0.582,500),
                    rep(0.686,500),rep(0.908,500),rep(0.582,500),
                    rep(0.582,500))

prob_dead_from_icu<-prob_ICU<- cbind(rep(0,500),rep(0.041,500),rep(0.071,500),
                    rep(0.217,500),rep(0.445,500),rep(0.071,500),
                    rep(0.071,500))

prob_vent_icu<- cbind(rep(0,500),rep(0.035,500),rep(0.059,500),
                    rep(0.214,500),rep(0.912,500),rep(0.059,500),
                    rep(0.059,500))

prob_dead_from_vent<- cbind(rep(0,500),rep(0.33,500),rep(0.25,500),
                    rep(0.692,500),rep(1.27,500),rep(0.25,500),
                    rep(0.25,500))

prob_array<- array(c(prob_severe,prob_hospitalized,prob_ICU,prob_dead_from_icu,
                     prob_vent_icu,prob_dead_from_vent),dim=c(500,7,6))
 ...
 for(i in 1:500){
   for(j in 2:ncol(data)){
     for(k in 1:365){
       ...
       counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,1]<-data[which(complete_data$run_index==i&complete_data$time==k),j,1]*prob_array[i,j-1,1]
       
       counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,2]<-counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,1]*prob_array[i,j-1,2]
       
        counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]<-counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,2]*prob_array[i,j-1,3]
        
       counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]<-counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]*prob_array[i,j-1,5]
    
   
    
     counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,5]<-(counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]*prob_array[i,j-1,4])+(counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]*prob_array[i,j-1,6])
             ...
             counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,2]<-counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,1]*prob_array[i,j-1,2]
              ...
              counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]<- 
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,2]*
      prob_array[i,j-1,3]
              
              counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]<- 
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-3)),j,3]*
      prob_array[i,j-1,5] 
         
          counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,5]<- 
      (counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]*
         prob_array[i,j-1,4])+
      (counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]*
         prob_array[i,j-1,6])
           ...
           counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,1]<- 
      data[which(complete_data$run_index==i&complete_data$time==(k-4)),j,1]*prob_array[i,j-1,1]
           
           counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,2]<- 
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,1]*prob_array[i,j-1,2]
               
         counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]<- 
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-4)),j,2]*prob_array[i,j-1,3]
         
          counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]<- 
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-3)),j,3]*prob_array[i,j-1,5] 
         
          counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,5]<- 
      (counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]*prob_array[i,j-1,4])+(counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,4]*prob_array[i,j-1,6])
      ...
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-3)),j,3]*prob_array[i,j-1,5] 
        ...
        counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,5]<- 
      (counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,3]*prob_array[i,j-1,4])+(counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-5)),j,4]*prob_array[i,j-1,6])
      }
      else{
      ...
      counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-3)),j,3]*prob_array[i,j-1,5] 
        ...
        counts_array_R0val[which(complete_data$run_index==i&complete_data$time==k),j,5]<- 
      (counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-8)),j,3]*prob_array[i,j-1,4])+(counts_array_R0val[which(complete_data$run_index==i&complete_data$time==(k-5)),j,4]*prob_array[i,j-1,6])
      }
     }
