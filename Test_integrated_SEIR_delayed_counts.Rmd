...
get_status_median<- function(combined_incident_median, median_cumulative_sum, n_sims, n_days,ICU_bed_capacity){
 ...
 combined_incident_median<- combined_incident_median%>%
   mutate(Icid_total= sum(Icid_Cat1,Icid_Cat2,Icid_Cat3,Icid_Cat4,Icid_Cat5,Icid_Cat6,Icid_Cat7))%>%
   mutate(Death_total= sum(Dead_Cat1,Dead_Cat2,Dead_Cat3,Dead_Cat4,Dead_Cat5,Dead_Cat6,Dead_Cat7))%>%
  mutate(Rolling_sum_Icid=roll_sum(Icid_total,3,fill=NA))%>%
  mutate(Rolling_sum_Death=roll_sum(Death_total,3,fill=NA))%>%mutate_all(~replace(., is.na(.), 0))%>%
  mutate(Rolling_mean_Icid=(Rolling_sum_Icid/3))%>%
  mutate(Rolling_mean_Death=(Rolling_sum_Death/3))
  ...
  shifted_combined_incident<- combined_incident_median%>%
    select(Day,Rolling_mean_Icid,Rolling_mean_Death)%>%
    mutate(New_Day=Day+1)
  
  check_red_status_death_icid<- left_join(combined_incident_median,shifted_combined_incident,by=c("Day"="New_Day"))%>%
    mutate_all(~replace(., is.na(.), 0))%>%
    mutate(red_status_death=ifelse(Rolling_mean_Death.y<=Rolling_mean_Death.x,1,0))%>%
             mutate(red_status_icid=ifelse(Rolling_mean_Icid.y<=Rolling_mean_Icid.x,1,0))%>%
    mutate(decline_death=ifelse(Rolling_mean_Death.y>Rolling_mean_Death.x,1,0))%>%
...
median_status_2C<-get_status_median(nedian_combined_incident_2C,median_cumulative_sums_2C,n_sims,n_days,600)
