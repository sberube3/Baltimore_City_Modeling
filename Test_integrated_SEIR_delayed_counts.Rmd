...
combined_sums_death_icid<- combined_sums_death_icid%>%
  ...
  shifted_combined_incident<- combined_sums_death_icid%>%
    select(Sim_index,Day,Icid_total,Death_total)%>%
  ...
  check_red_status_death_icid<- left_join(combined_sums_death_icid,shifted_combined_incident,by=c("Day"="New_Day","Sim_index"="Sim_index"))%>%
    mutate_all(~replace(., is.na(.), 0))%>%
    mutate(red_status_death=ifelse(Death_total.y<=Death_total.x,1,0))%>%
             mutate(red_status_icid=ifelse(Icid_total.y<=Icid_total.x,1,0))%>%
    mutate(decline_death=ifelse(Death_total.y>Death_total.x,1,0))%>%
    mutate(decline_icid=ifelse(Icid_total.y>Icid_total.x,1,0))%>%
