...
counts_array_A<- incident_classifications(combined_incident_A,complete_scenarioA,A,prob_matrix)
counts_array_B<- incident_classifications(combined_incident_B,complete_scenarioB,B,prob_matrix)
counts_array_C<- incident_classifications(combined_incident_C,complete_scenarioC,C,prob_matrix)
counts_array_D<- incident_classifications(combined_incident_D,complete_scenarioD,D,prob_matrix)
counts_array_E<- incident_classifications(combined_incident_E,complete_scenarioE,E,prob_matrix)

counts_array_asymp_A<- incident_classifications(combined_incident_asymp_A,complete_scenarioA,A,prob_matrix)
counts_array_asymp_B<- incident_classifications(combined_incident_asymp_B,complete_scenarioB,B,prob_matrix)
counts_array_asymp_C<- incident_classifications(combined_incident_asymp_C,complete_scenarioC,C,prob_matrix)
counts_array_asymp_D<- incident_classifications(combined_incident_asymp_D,complete_scenarioD,D,prob_matrix)
counts_array_asymp_E<- incident_classifications(combined_incident_asymp_E,complete_scenarioE,E,prob_matrix)
...
cumulative_array_A<- cumulative_counts(counts_array_A,complete_scenarioA,A)
cumulative_array_B<- cumulative_counts(counts_array_B,complete_scenarioB,B)
cumulative_array_C<- cumulative_counts(counts_array_C,complete_scenarioC,C)
cumulative_array_D<- cumulative_counts(counts_array_D,complete_scenarioD,D)
cumulative_array_E<- cumulative_counts(counts_array_E,complete_scenarioE,E)

cumulative_array_asymp_A<- cumulative_counts(counts_array_asymp_A,complete_scenarioA,A)
cumulative_array_asymp_B<- cumulative_counts(counts_array_asymp_B,complete_scenarioB,B)
cumulative_array_asymp_C<- cumulative_counts(counts_array_asymp_C,complete_scenarioC,C)
cumulative_array_asymp_D<- cumulative_counts(counts_array_asymp_D,complete_scenarioD,D)
cumulative_array_asymp_E<- cumulative_counts(counts_array_asymp_E,complete_scenarioE,E)
...
calc_1 <- f_1_asymp[[6]] %>% dplyr::group_by(scenario,summary_stat) %>% dplyr::filter(Time<=as.Date(end_date)) %>%
  dplyr::summarise(incident_symptomatic=max(incident_symptomatic,na.rm=TRUE)) %>%
  ungroup %>% left_join(.,f_1_asymp[[6]])
...
calc_4<- est_fig_4(f_1[[1]],f_1[[2]],f_1[[3]],f_1[[4]],f_1[[5]],
                  ...
                  upper_cumulative_array_D,upper_cumulative_array_E,
                  cumulative_array_A,cumulative_array_B,cumulative_array_C,cumulative_array_D,cumulative_array_E)
...
plt_cum_deaths<- ggplot() + 
  ...
  theme_bw() + scale_x_date(date_labels = "%b", date_breaks = "2 month", date_minor_breaks = "1 month",limits = as.Date(c(NA,end_date))) + scale_color_brewer(palette="Dark2") + ylim(0,26000)
