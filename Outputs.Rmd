...
(1) We assume age-structured mixing in the population and seed the epidemic with 8 asymptomatic cases and 2 symptomatic cases. Healthcare workers and the homeless population are considered as explicit populations that follow the mixing patterns of the 45-54 year age category.  
(2) We assume the age structure of Baltimore City using data from the 2010 Baltimore census, and assume the population of healthcare workers is 5,000 and the population of individuals who are homeless is 3,000. 
(3) Using current estimates from the CDC, we assume age-specific probabilities of becoming a hospitalized case and getting admitted to the ICU if hospitalized (https://www.cdc.gov/mmwr/volumes/69/wr/mm6912e2.htm#T1_down). 
(4) Using current statistics from the WHO, we assume 25% of cases in ICU across age groups get put on a ventilator.
(5) Based on expert clinical opinion and similar estimates used in Great Britain, we assume 50% of those on a ventilator die and an age-dependent proportion of those that do not require a ventilator die (calculated to match the CDC age-specific CFR). 
(6) For time to ventilator use, hospital discharge, and death, we apply an 11 day lag based on data from China.
(7) We assume healthcare workers and those who are homeless have the same hospitalization, ICU, ventilator, and death probabilities as those in the 45-54 age category. 
(8) For the figures and tables here, we assume the epidemic started March 1st, 2020.
...
#### Daily incident symptomatic infections across all population groups
...
combined_long_matrix<-combined_long_matrix%>%
  ...
  mutate(Day= rep(seq(as.Date('2020/03/1'),by='day', 
                      length.out=length(incident_cases_R017_75)),times=3))

fig_nums(name = paste0("fig-one"), caption = paste("Plot of the median number of incident cases across simulations by day for each transmission scenario. These estimates are summed across all age groups, and the healthcare worker and homeless populations."), display = "false")
```
...
#### Daily incident hospitalizations across all population groups
...
fig_nums(name = paste0("fig-two"), caption = paste("Plot of the median number of incident hospitalizations across simulations by day for each transmission scenario. These estimates are summed across all age groups, and the healthcare worker and homeless populations."), display = "false")
```
...
#### Daily incident deaths across all population groups
...
fig_nums(name = paste0("fig-three"), caption = paste("Plot of the median number of deaths across simulations by day for each transmission scenario. These estimates are summed across all age groups, and the healthcare worker and homeless populations."), display = "false")
```
...
######EXTRA FIGURE. NOT PLOTTED IN REPORT.
#Figure 4 Cumulative hospitalizations
...
#### Considering new daily infections, what are the concurent number of hospital beds, ICU beds, and ventilators in use? 
...
plt_cumulative_hosp<- ggplot(cumulative_hosp_long)+
  geom_line(aes(x=Day,y=cumulative_hosp,color=str_wrap(scenario,10)))+
  labs(y="Number of hospital beds", title="Daily Hospital Bed Use") + labs(color="Age group")+   theme(legend.title = element_text(size = 7),legend.text = element_text(size = 7))
...
plt_cumulative_ICU<- ggplot(cumulative_ICU_long)+
  geom_line(aes(x=Day,y=cumulative_ICU,color=str_wrap(scenario,10)))+
  labs(y="Number of ICU beds", title="Daily ICU Bed Use") + labs(color="Age group")+   theme(legend.title = element_text(size = 7),legend.text = element_text(size = 7))
...
plt_cumulative_vent<- ggplot(cumulative_vent_long)+
  geom_line(aes(x=Day,y=cumulative_vent_use,color=str_wrap(scenario,10)))+
  labs(y="Number of Vents", title="Daily Vent Use") + labs(color="Age group") +   theme(legend.title = element_text(size = 7),legend.text = element_text(size = 7))

pltC<- plt_cumulative_vent


fig_nums(name = paste0("fig-four"), caption = paste("A. Plot of daily hospitalizations for each transmission scenario across all age categories, and the healthcare worker and homeless populations. B. Plot of daily ICU admissions for each transmission scenario across all age categories, and the healthcare worker and homeless populations. C. Plot of number of daily ventilators needed for each transmission scenario across all age categories, and the healthcare worker and homeless populations."), display = "false")

```

`r fig_nums("fig-four", display = "full")`

```{r, fig-four, fig.dim = c(12,4.5)}

plt <- cowplot::plot_grid(pltA, pltB, pltC, labels=c('A','B','C'), align = "h", ncol=3)
print(plt)
...
tab_nums(name = "table_one", caption = "Estimates of the peak number of total infections, hospitalizations, ICU beds in use, ventilators in use, and deaths in a single day by month and transmission scenario. We present the median and 25th and 75th percentiles across all simulations and sum across all population categories.", display = FALSE)
```
...
tab_nums(name = "table_two", caption = "Estimates of the peak number of total infections, hospitalizations, ICU beds in use, ventilators in use, and deaths in a single day by month, population category, and transmission scenario. We present the median and 25th and 75th percentiles across all simulations.", display = FALSE)
```
