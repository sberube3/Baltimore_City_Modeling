...
(2) Using current estimates from the CDC, we assume age-specific probabilities of becoming a hospitalized case and getting admitted to the ICU if hospitalized (https://www.cdc.gov/mmwr/volumes/69/wr/mm6912e2.htm#T1_down). 
(3) Using current statistics from the WHO, we assume 25% of cases in ICU across age groups get put on a ventilator.
(4) Based on expert clinical opinion and similar estimates used in Great Britain, we assume 50% of those on a ventilator die and an age-dependent proportion of those that do not require a ventilator die (calculated to match the CDC age-specific CFR).
(5) We assume healthcare workers and those who are homeless have the same hospitalization, ICU, ventilator probabilities as those in the 45-54 age category. 
