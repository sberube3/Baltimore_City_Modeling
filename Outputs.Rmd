...
- Moderate transmission scenario where $R_0$ is assumed to be between 2-3.
- High transmission scenario where $R_0$ is assumed to be between 3.5-4.
...
```{r, cache=FALSE, message=FALSE, echo=TRUE, warning=FALSE}
#subset
...
counts_array_R0lowA<- incident_classifications( combined_incident_R0lowA, complete_R0lowA,R0lowA)
counts_array_R0lowB<- incident_classifications( combined_incident_R0lowB, complete_R0lowB,R0lowB)
counts_array_R0lowC<- incident_classifications( combined_incident_R0lowC, complete_R0lowC,R0lowC)
counts_array_R0highA<- incident_classifications( combined_incident_R0highA, complete_R0highA,R0highA)
counts_array_R0highB<- incident_classifications( combined_incident_R0highB, complete_R0highB,R0highB)
counts_array_R0highC<- incident_classifications( combined_incident_R0highC, complete_R0highC,R0highC)
...
est_fig_1<- function(complete_data_1,complete_data_2,complete_data_3,
                      median_data_1,median_data_2,median_data_3,
                      combined_data_1,combined_data_2,combined_data_3){
random_sample_R017<- which(complete_data_1$run_index%in%sample(rep(1:50),15,replace=F))
random_sample_R021<- which(complete_data_2$run_index%in%sample(rep(1:50), 15, replace=F))
...
symptomatic_df_sep<- symptomatic_df%>%
  ...
  mutate(scenario=ifelse(scenario=="R_0=1.7", "Uncontrolled",
                         ifelse(scenario=="R_0=2.1", "Mildly restrictive", 
                                "Moderately restrictive")))%>%
  mutate(Day=rep(seq(as.Date('2020/02/1'),by='day',length.out=n_days),48))
  return(symptomatic_df_sep)
}

f_1_low<- make_fig_1( complete_R0lowA, complete_R0lowB, complete_R0lowC,
...
f_1_high<- make_fig_1( complete_R0highA, complete_R0highB, complete_R0highC,
...
calc_1_low<- est_fig_1(complete_scenario1A,complete_scenario1B,complete_scenario1C,
           median_incident_symptomatic_R0lowA,median_incident_symptomatic_R0lowB,
           median_incident_symptomatic_R0lowC,
           combined_incident_R0lowA,combined_incident_R0lowB,combined_incident_R0lowC) 

est_1_low <- calc_1_low %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_symptomatic=max(incident_symptomatic,na.rm=TRUE),min=min(incident_symptomatic,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_1_low)

calc_1_high<- est_fig_1(complete_scenario2A,complete_scenario2B,complete_scenario2C,
           median_incident_symptomatic_R0highA,median_incident_symptomatic_R0highB,
           median_incident_symptomatic_R0highC,
           combined_incident_R0highA,combined_incident_R0highB,combined_incident_R0highC) 

est_1_high <- calc_1_high %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_symptomatic=max(incident_symptomatic,na.rm=TRUE),min=min(incident_symptomatic,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_1_high)
...
- In the moderate transmission scenario, the peak when no intervention strategies are used is expected May 16 with 3,567 incident infections. 
- When a mildly restrictive intervention strategy is employed, the peak is shifted and expected to occur June 10 with 3,499	 daily incident infections. 
- When a moderately restrictive intervention strategy is employed, we see minimal cases.
</div></div>
...
<div class = "row"><div class = "col-md-6"><div class = "blue">
- In the high transmission scenario, the peak when no intervention strategies are used is expected to have already occurred on April 10 with 6,735 incidenct infection, which is markedly higher compared to the low transmission scenario. 
...
- When a moderately restrictive intervention strategy is employed, the epidemic is slow to take off and remains relatively flat with 723 peak incident infections.
</div></div>
...
est_fig_2<- function(random_sample_1,random_sample_2,random_sample_3,
                      counts_data_1,counts_data_2,counts_data_3,
                      median_counts_data_1,median_counts_data_2,median_counts_data_3){
incident_hosp_wide<- as.data.frame(cbind(median_counts_data_1[,1,1],
                            apply(median_counts_data_1[,-1,1],1,sum),
...
calc_2_low<- est_fig_2(f_1_low[[2]],f_1_low[[3]],f_1_low[[4]],
                 counts_array_R0lowA,counts_array_R0lowB,counts_array_R0lowC,
                 median_counts_array_R0lowA,median_counts_array_R0lowB,median_counts_array_R0lowC) 

est_2_low <- calc_2_low %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_hosp=max(incident_hosp,na.rm=TRUE),min=min(incident_hosp,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_2_low)

calc_2_high<- est_fig_2(f_1_high[[2]],f_1_high[[3]],f_1_high[[4]],
                 counts_array_R0highA,counts_array_R0highB,counts_array_R0highC,
                 median_counts_array_R0highA,median_counts_array_R0highB,median_counts_array_R0highC) 

est_2_high <- calc_2_high %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_hosp=max(incident_hosp,na.rm=TRUE),min=min(incident_hosp,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_2_high)
...
<div class = "row"><div class = "col-md-6"><div class = "blue">
- In the low transmission scenario, the peak of incident hospitalizations when no intervention strategies are used is May 20 with 1,807 incident hospitalizations. 
- When a mildly restrictive intervention strategy is employed, the peak is shifted and expected to occur June 14 with 1,772 incident hospitalizations. 
</div></div>
...
<div class = "row"><div class = "col-md-6"><div class = "blue">
- In the high transmission scenario, the peak of incident hospitalizations when no intervention strategies are used is expected April 15 with 3,430 incident hospitalizations. 
- When a mildly restrictive intervention strategy is employed, the peak is shifted and expected to occur May 9 with 3,126 incident hospitalizations. 
- When a moderately restrictive intervention strategy is employed, the epidemic is slow to take off and hospitalizations reach a peak in June with a maximum of 621 incident hospitalizations.
</div></div>
...
est_fig_3<- function(random_sample_1,random_sample_2,random_sample_3,
                      counts_data_1,counts_data_2,counts_data_3,
                      median_counts_data_1,median_counts_data_2,median_counts_data_3){
incident_death_wide<- as.data.frame(cbind(median_counts_data_1[,1,5],
                            apply(median_counts_data_1[,-1,5],1,sum),
...
calc_3_low<- est_fig_3(f_1_low[[2]],f_1_low[[3]],f_1_low[[4]],
                 counts_array_R0lowA,counts_array_R0lowB,counts_array_R0lowC,
                 median_counts_array_R0lowA,median_counts_array_R0lowB,median_counts_array_R0lowC)

est_3_low <- calc_3_low %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_death=max(incident_death,na.rm=TRUE),min=min(incident_death,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_3_low)

calc_3_high<- est_fig_3(f_1_high[[2]],f_1_high[[3]],f_1_high[[4]],
                 counts_array_R0highA,counts_array_R0highB,counts_array_R0highC,
                 median_counts_array_R0highA,median_counts_array_R0highB,median_counts_array_R0highC)

est_3_high <- calc_3_high %>% dplyr::group_by(scenario,summary_stat) %>% 
  dplyr::summarise(incident_death=max(incident_death,na.rm=TRUE),min=min(incident_death,na.rm=TRUE)) %>% ungroup %>% left_join(.,calc_3_high)
...
<div class = "row"><div class = "col-md-6"><div class = "blue">
- In the low transmission scenario, the peak of incident deaths when no intervention strategies are used is expected June 1 with 1,018 incident deaths. 
- When a mildly restrictive intervention strategy is employed, the peak is shifted and expected to occur June 30 with 1,005 incident deaths. 
- With a moderately restrictive intervention strategy, we expect to see a maximum of 38 incident deaths by July end.
</div></div>
...
<div class = "row"><div class = "col-md-6"><div class = "blue">
- In the high transmission scenario, the peak of incident deaths when no intervention strategies are used is expected April 27 with 1,962 incident deaths. 
- When a mildly restrictive intervention strategy is employed, the peak is shifted and expected to occur May 21 with 1,792 incident deaths.
- With a moderately restrictive intervention strategy, we expect to see a maximum of 349 incident deaths by June end.
</div></div>
...
<div class = "row"><div class = "row-md-6"><div class = "blue">
- These results suggest that in a high transmission scenario with no intervention or a mildly restrictive intervention strategy,  hospital capacity may be reached before May. 
- If we assume there are 600 ventilators in Baltimore City, we may reach ventilator capacity by April-end if no intervention or mildly restrictive intervention strategies are imposed.
- With moderately restrictive intervention strategies, capacity may not be reached until the late July in the high transmission scenario.
</div></div>
