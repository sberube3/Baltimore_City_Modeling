...
We compare four intervention strategies under a moderate transmission scenario, where $R_0$ is assumed to be between 2-3 (each person infected with the disease will transmit the disease to 2-3 people on average), for model simulations from March 1, 2020 through December 31, 2020. The intervention strategies considered were: 
...
##load data
setwd("~/Dropbox/Postdoc/COVID19/COVID-BaltimoreCity/output_20200514")
complete_scenarioA <- read.csv("scenario1.csv")
complete_scenarioB <- read.csv("scenario2.csv")
complete_scenarioC <- read.csv("scenario3.csv")
complete_scenarioD <- read.csv("scenario4.csv")
...
#only consider first 500 sims 
combined_incident_A<- combined_incident_A[1:(500*n_days),]
combined_incident_B<- combined_incident_B[1:(500*n_days),]
combined_incident_C<- combined_incident_C[1:(500*n_days),]
combined_incident_D<- combined_incident_D[1:(500*n_days),]

combined_incident_asymp_A<- combined_incident_asymp_A[1:(500*n_days),]
combined_incident_asymp_B<- combined_incident_asymp_B[1:(500*n_days),]  
combined_incident_asymp_C<- combined_incident_asymp_C[1:(500*n_days),]   
combined_incident_asymp_D<- combined_incident_asymp_D[1:(500*n_days),]  

complete_scenarioA<- complete_scenarioA[1:(500*n_days),]
complete_scenarioB<- complete_scenarioB[1:(500*n_days),]
complete_scenarioC<- complete_scenarioC[1:(500*n_days),]
complete_scenarioD<- complete_scenarioD[1:(500*n_days),]
...
make_fig_1<- function(complete_data_1,complete_data_2,complete_data_3,complete_data_4,
                      median_data_1,median_data_2,median_data_3,median_data_4,
                      combined_data_1,combined_data_2,combined_data_3,combined_data_4){
random_sample_A<- which(complete_data_1$run_index%in%sample(rep(1:500),15,replace=F))
random_sample_B<- which(complete_data_2$run_index%in%sample(rep(1:500), 15, replace=F))
random_sample_C<- which(complete_data_3$run_index%in%sample(rep(1:500), 15, replace = F))
random_sample_D<- which(complete_data_4$run_index%in%sample(rep(1:500), 15, replace = F))
...
colnames(incident_symptomatic_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median") 
...
colnames(random_sims)<- c("Time", "A,Random", "B,Random", "C,Random", "D,Random")

incident_symptomatic_long<- incident_symptomatic_wide%>%
  gather(scenario, incident_symptomatic, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, incident_symptomatic, c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)

df_sep<- incident_symptomatic_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  dplyr::mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),4))

random_df_sep<- random_sims_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  dplyr::mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),60), 
         number=c(rep(1:15,each=n_days),rep(1:15,each=n_days),rep(1:15,each=n_days),
                  rep(1:15,each=n_days)))
...
return(list(random_sample_A,random_sample_B,random_sample_C,random_sample_D,df_sep,random_df_sep,cumsum_cases))
}


f_1<- make_fig_1(complete_scenarioA,complete_scenarioB,complete_scenarioC,complete_scenarioD, 
           median_incident_symptomatic_A,median_incident_symptomatic_B,median_incident_symptomatic_C,
           median_incident_symptomatic_D,
           combined_incident_A,combined_incident_B,combined_incident_C,combined_incident_D)

f_1_asymp<- make_fig_1(complete_scenarioA,complete_scenarioB,complete_scenarioC,complete_scenarioD, 
           incident_asymptomatic_A[[1]],incident_asymptomatic_B[[1]],incident_asymptomatic_C[[1]],
           incident_asymptomatic_D[[1]],
           combined_incident_asymp_A,combined_incident_asymp_B,combined_incident_asymp_C,
           combined_incident_asymp_D)


########plots

plt_incident_cases<- ggplot() + 
  geom_line(data=f_1[[5]], aes(y = incident_symptomatic, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=f_1[[6]], aes(y = incident_symptomatic, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
f_1[[5]]<- as.data.frame(cbind(f_1[[5]], rep(0,nrow(f_1[[5]]))))
colnames(f_1[[5]])<- c("Time", "scenario", "summary_stat", "incident_symptomatic",
                       "number")
incident_cases_df<- as.data.frame(rbind(f_1[[5]],f_1[[6]]))
write.csv(incident_cases_df, "Figure_1_incident_cases.csv")

plt_incident_cases_total<- ggplot() + 
  geom_line(data=f_1_asymp[[5]] ,aes(y = incident_symptomatic, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=f_1_asymp[[6]] ,aes(y = incident_symptomatic, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
f_1_asymp[[5]]<- as.data.frame(cbind(f_1_asymp[[5]],rep(0,nrow(f_1_asymp[[5]]))))
colnames(f_1_asymp[[5]])<- c("Time", "scenario", "summary_stat", "incident_symptomatic",
                       "number")
incident_cases_asymp_df<- as.data.frame(rbind(f_1_asymp[[5]],f_1_asymp[[6]]))
write.csv(incident_cases_asymp_df, "Figure_1_incident_cases_asymp.csv")
 


########peak estimates table
#change inputs from median to lower or upper to get bounds
calc_1 <- f_1[[5]] %>% dplyr::group_by(scenario,summary_stat) %>% dplyr::filter(Time<=as.Date(end_date)) %>%
  dplyr::summarise(incident_symptomatic=max(incident_symptomatic,na.rm=TRUE)) %>%
  ungroup %>% left_join(.,f_1[[5]])

calc_asymp_1 <- f_1_asymp[[5]] %>% dplyr::group_by(scenario,summary_stat) %>% dplyr::filter(Time<=as.Date(end_date)) %>%
  dplyr::summarise(incident_symptomatic=max(incident_symptomatic,na.rm=TRUE)) %>%
  ungroup %>% left_join(.,f_1_asymp[[5]])
...
make_fig_2<- function(random_sample_1,random_sample_2,random_sample_3,random_sample_4,
                      counts_data_1,counts_data_2,counts_data_3,counts_data_4,
                      median_counts_data_1,median_counts_data_2,median_counts_data_3,
                      median_counts_data_4){
...
colnames(incident_hosp_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median")  
...
incident_hosp_long<- incident_hosp_wide%>%
  gather(scenario, incident_hosp, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, incident_hosp, c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)

df_sep<- incident_hosp_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),4))

random_df_sep<- random_sims_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),60), 
         number=c(rep(1:15,each=n_days),rep(1:15,each=n_days),rep(1:15,each=n_days),
                  rep(1:15,each=n_days)))
...
f_2 <- make_fig_2(f_1[[1]],f_1[[2]],f_1[[3]],f_1[[4]],
                  counts_array_A,counts_array_B,counts_array_C,counts_array_D,
                  median_counts_array_A,median_counts_array_B,median_counts_array_C,
                  median_counts_array_D)


########plots
plt_hosp_cases<- ggplot() + 
  geom_line(data=f_2[[1]] ,aes(y = incident_hosp, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=f_2[[2]] ,aes(y = incident_hosp, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
f_2[[1]]<- as.data.frame(cbind(f_2[[1]], rep(0,nrow(f_2[[1]]))))
colnames(f_2[[1]])<- c("Time", "scenario", "summary_stat", "incident_hosp",
                       "number")
hosp_cases_df<- as.data.frame(rbind(f_2[[1]],f_2[[2]]))
write.csv(hosp_cases_df, "Figure_2_incident_hosp.csv")
...
make_fig_3<- function(random_sample_1,random_sample_2,random_sample_3,random_sample_4,
                      counts_data_1,counts_data_2,counts_data_3,counts_data_4,
                      median_counts_data_1,median_counts_data_2,median_counts_data_3,
                      median_counts_data_4){
...
colnames(incident_death_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median")
...
incident_death_long<- incident_death_wide%>%
  gather(scenario, incident_death, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, incident_death, c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)
...
f_3<- make_fig_3(f_1[[1]],f_1[[2]],f_1[[3]],f_1[[4]],
                  counts_array_A,counts_array_B,counts_array_C,counts_array_D,
                  median_counts_array_A,median_counts_array_B,median_counts_array_C,
                  median_counts_array_D)
...
plt_deaths<- ggplot() + 
  geom_line(data=f_3[[1]] ,aes(y = incident_death, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=f_3[[2]] ,aes(y = incident_death, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
  labs(y="Incident Deaths",x="Time in Days") +
  #labs(color="Strategy",linetype="Summary Statistic") +
  theme(axis.text.x = element_text(face="bold", color="#993333", size=20, angle=45),
          axis.text.y = element_text(face="bold",size=20)) + theme_bw() + facet_grid(.~scenario) + guides(color=FALSE) + scale_x_date(date_labels = "%b", date_breaks = "2 months", date_minor_breaks = "1 month",limits = as.Date(c(NA,end_date))) +
  theme(strip.background = element_blank(), strip.text.x = element_text(size = 12,face = "bold.italic")) + scale_color_brewer(palette="Dark2") #+ ylim(0,20) #+ scale_y_sqrt(breaks = c(100, 1000, 2500)) 

f_3[[1]]<- as.data.frame(cbind(f_3[[1]], rep(0,nrow(f_3[[1]]))))
colnames(f_3[[1]])<- c("Time", "scenario", "summary_stat", "incident_death",
                       "number")
death_cases_df<- as.data.frame(rbind(f_3[[1]],f_3[[2]]))
write.csv(death_cases_df, "Figure_3_incident_death.csv")
...
make_fig_4<- function(random_sample_1,random_sample_2,random_sample_3,random_sample_4,
                      cumulative_median_1,cumulative_median_2,cumulative_median_3,cumulative_median_4,
                      cumulative_data_1,cumulative_data_2,cumulative_data_3,cumulative_data_4){
...
colnames(cumulative_hosp_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median") 
...
cumulative_hosp_long<- cumulative_hosp_wide%>%
  gather(scenario, cumulative_hosp, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, cumulative_hosp,c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)
...
plt_cumulative_hosp<- ggplot() + 
  geom_line(data=df_sep,aes(y = cumulative_hosp, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_sep,aes(y = cumulative_hosp, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
colnames(cumulative_ICU_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median") 
...
cumulative_ICU_long<- cumulative_ICU_wide%>%
  gather(scenario, cumulative_ICU, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, cumulative_ICU, c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)

df_sep_ICU<- cumulative_ICU_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),4))

random_df_sep_ICU<- random_sims_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),60), 
         number=c(rep(1:15,each=n_days),rep(1:15,each=n_days),rep(1:15,each=n_days),
                  rep(1:15,each=n_days)))
...
plt_cumulative_ICU<- ggplot() + 
  geom_line(data=df_sep_ICU,aes(y = cumulative_ICU, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_sep_ICU,aes(y = cumulative_ICU, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
colnames(cumulative_vent_wide)<- c("Time", "A,Median", "B,Median", "C,Median", "D,Median") 
...
cumulative_vent_long<- cumulative_vent_wide%>%
  gather(scenario, cumulative_vent, c("A,Median", "B,Median", "C,Median", "D,Median"), factor_key = TRUE)

random_sims_long<- random_sims%>%
  gather(scenario, cumulative_vent, c("A,Random", "B,Random", "C,Random", "D,Random"), factor_key=TRUE)

df_sep_vent<- cumulative_vent_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),4))

random_df_sep_vent<- random_sims_long %>%
  separate(scenario, into=c("scenario", "summary_stat"), sep=",")%>%
  mutate(Time=rep(seq(as.Date(start_date),by='day',length.out=n_days),60), 
         number=c(rep(1:15,each=n_days),rep(1:15,each=n_days),rep(1:15,each=n_days),
                  rep(1:15,each=n_days)))
...
plt_cumulative_vent<- ggplot() + 
  geom_line(data=df_sep_vent,aes(y = cumulative_vent, x=Time,color=str_wrap(scenario,10)), size=1.2)+
  geom_line(data=random_df_sep_vent,aes(y = cumulative_vent, x=Time, group=number,color=str_wrap(scenario,10)),alpha=0.2, size=0.75)+
...
return(list(plt_cumulative_hosp,plt_cumulative_ICU,plt_cumulative_vent,df_sep,random_df_sep,df_sep_ICU,random_df_sep_ICU,df_sep_vent,random_df_sep_vent))

}

f_4 <- make_fig_4(f_1[[1]],f_1[[2]],f_1[[3]],f_1[[4]],
                  median_cumulative_array_A,median_cumulative_array_B,median_cumulative_array_C,
                  median_cumulative_array_D,
                  cumulative_array_A,cumulative_array_B,cumulative_array_C,cumulative_array_D)

f_4[[4]]<- as.data.frame(cbind(f_4[[4]], rep(0,nrow(f_4[[4]]))))
colnames(f_4[[4]])<- c("Time", "scenario", "summary_stat", "cumulative_hosp",
                       "number")
cumulative_hosp_df<- as.data.frame(rbind(f_4[[4]],f_4[[5]]))
write.csv(cumulative_hosp_df, "Figure_4_cumulative_hosp.csv")

f_4[[6]]<- as.data.frame(cbind(f_4[[6]], rep(0,nrow(f_4[[6]]))))
colnames(f_4[[6]])<- c("Time", "scenario", "summary_stat", "cumulative_ICU",
                       "number")
cumulative_ICU_df<- as.data.frame(rbind(f_4[[6]],f_4[[7]]))
write.csv(cumulative_ICU_df, "Figure_4_cumulative_ICU.csv")

f_4[[8]]<- as.data.frame(cbind(f_4[[8]], rep(0,nrow(f_4[[8]]))))
colnames(f_4[[8]])<- c("Time", "scenario", "summary_stat", "cumulative_vent",
                       "number")
cumulative_vent_df<- as.data.frame(rbind(f_4[[8]],f_4[[9]]))
write.csv(cumulative_vent_df, "Figure_4_cumulative_vent.csv")
...
########peak estimates table
est_fig_4<- function(random_sample_1,random_sample_2,random_sample_3,random_sample_4,
                      cumulative_median_1,cumulative_median_2,cumulative_median_3,cumulative_median_4,
                      cumulative_data_1,cumulative_data_2,cumulative_data_3,cumulative_data_4){
 cumulative_hosp_wide<- as.data.frame(cbind(cumulative_median_1[,1,2],
                            apply(cumulative_median_1[,-1,2],1,sum),
...
calc_4<- est_fig_4(f_1[[1]],f_1[[2]],f_1[[3]],f_1[[4]],
                  upper_cumulative_array_A,upper_cumulative_array_B,upper_cumulative_array_C,
                  upper_cumulative_array_D,
                  cumulative_array_A,cumulative_array_B,cumulative_array_C,cumulative_array_D)
...
plt_cum_cases<- ggplot() + 
  geom_line(data=f_1[[7]],aes(y = cum_symp, x=Time,color=scenario), size=1.2)+
  labs(y="Cumulative Symptomatic Cases",x="Time in Days") + theme(text = element_text(size=20)) +
  theme_bw() + scale_x_date(date_labels = "%b", date_breaks = "2 month", date_minor_breaks = "1 month",limits = as.Date(c(NA,end_date))) + scale_color_brewer(palette="Dark2") + ylim(0,250000)

write.csv(f_1[[7]],"Figure_5_cumulative_cases.csv")
...
plt_cum_deaths<- ggplot() + 
  ...
  theme_bw() + scale_x_date(date_labels = "%b", date_breaks = "2 month", date_minor_breaks = "1 month",limits = as.Date(c(NA,end_date))) + scale_color_brewer(palette="Dark2") + ylim(0,7500)

write.csv(f_3[[3]],"Figure_5_cumulative_deaths.csv")

cowplot::plot_grid(plt_cum_cases, plt_cum_deaths, labels=c('A','B'), align = "h", nrow=2)

########peak estimates table
est_fig_5<- function(combined_data_1,combined_data_2,combined_data_3,combined_data_4,
                      cumulative_data_1,cumulative_data_2,cumulative_data_3,cumulative_data_4){
daily_incident_cases<- matrix(NA,nrow(combined_data_1),ncol=5)
...
median_cumulative_cases_symptomatic<- matrix(NA, n_days, ncol= 5)
...
colnames(cumulative_cases_symptomatic)<- c("Time", "A","B","C","D")
cumulative_cases_symptomatic_long<- cumulative_cases_symptomatic%>%
  gather(scenario, cumulative_cases, c("A","B","C","D"), factor_key = T)%>%
  mutate(Time= rep(seq(as.Date(start_date),by="day", length.out = n_days),4))
...
median_cumulative_deaths<- matrix(NA, n_days, 5)
...
colnames(median_cumulative_deaths)<- c("Time", "A","B","C","D")
median_cumulative_deaths_long<- median_cumulative_deaths%>%
  gather(scenario, cumulative_death, c("A","B","C","D"), factor_key = T)%>%
   mutate(Time= rep(seq(as.Date(start_date),by="day", length.out = n_days),4))
...
calc_5<- est_fig_5(combined_incident_A,combined_incident_B,combined_incident_C,combined_incident_D,cumulative_array_A,cumulative_array_B,cumulative_array_C,combined_incident_D)
